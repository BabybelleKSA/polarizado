...
export default function LeadForm({ carType, tintPercent, price }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMessage({
          type: 'success',
          text: `Request sent! Diaz Polarizado will reach out soon. ${carType} | ${tintPercent}% | $${displayPrice}`
        });
  ...
  return (
    <div className="glass-card rounded-3xl p-6 sm:p-7 shadow-[0_24px_80px_rgba(0,0,0,0.55)] fade-up relative overflow-hidden border border-white/10">
      ...
      <div className="space-y-1 pb-2">
        <p className="text-xs uppercase tracking-[0.3em] text-white/60">Book your slot</p>
        <h2 className="text-2xl font-semibold text-white">Request My Tint</h2>
      </div>
      ...
      <form className="flex flex-col gap-4 pt-2" onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col gap-2 text-sm font-semibold text-white/80">
          <span className="text-white">Preferred Contact Method</span>
        ...
        >
          ...
          {submitting ? 'Sending...' : 'Request My Tint'}
        </button>
