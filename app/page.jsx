    ...
    <main className="min-h-screen bg-luxury-black text-white px-4 py-12 sm:py-16 lg:py-20">
        ...
        <div className="relative grid max-w-6xl grid-cols-1 gap-12 lg:grid-cols-12 lg:items-center lg:gap-10 xl:gap-14 mx-auto">
          <div className="lg:col-span-6 xl:col-span-5 space-y-6 fade-up relative">
            ...
            <div className="space-y-3 headline-glow">
              <p className="text-xs uppercase tracking-[0.32em] text-white/60">Premium Automotive Tinting</p>
              <div className="relative sweep">
                <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black leading-tight tracking-tight text-white relative z-10">
                  Crafted for Clarity.
                  <br />
                  Engineered for Heat Rejection.
                </h1>
              </div>
              <p className="text-lg text-white/70 max-w-xl">
                A cinematic, concierge-grade tint experience. Precision installs, zero compromise on comfort or style.
              </p>
            </div>
      ...
      <section id="pricing" className="mx-auto mt-16 max-w-6xl space-y-8 px-1">
        <div className="flex flex-col gap-2 fade-up">
          <p className="text-sm uppercase tracking-[0.28em] text-white/60">Transparent pricing</p>
          <h2 className="text-3xl sm:text-4xl font-black text-white tracking-tight">
            Build your tint in seconds.
          </h2>
          <p className="text-white/70 max-w-3xl">
            Choose your vehicle, pick your shade, and request your preferred time. We confirm quickly so you can get back to the drive.
          </p>
        </div>

        <div className="grid gap-6 lg:grid-cols-2">
          <Calculator onChange={setSelection} />
          <LeadForm
        ...
        <div id="packages" className="grid gap-4 pt-4 sm:grid-cols-2 lg:grid-cols-3 fade-up">
          {[
            {
              title: 'Basic Tint Package',
              price: '$199 - $299',
              perks: ['Dyed film', 'UV + glare reduction', 'Clean install']
            },
            {
              title: 'Premium Ceramic',
              price: '$349 - $499',
              perks: ['Ceramic heat rejection', 'Signal-friendly', 'Factory-match shade options']
            },
            {
              title: 'Ultra Ceramic IR',
              price: '$549 - $799',
              perks: ['Infrared heat blocking', 'Maximum clarity', 'Lifetime, transferable warranty']
            }
          ].map((pkg) => (
            <div
              key={pkg.title}
      ...
      <footer className="mx-auto mt-16 max-w-6xl rounded-2xl border border-white/5 bg-black/60 px-6 py-6 text-sm text-white/70 backdrop-blur">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="font-semibold tracking-wide text-white">Diaz Polarizado - Premium Tint Studio</div>
          <div className="flex flex-wrap gap-4 text-white/80">
            <span className="font-semibold text-white">Call/Text: (555) 321-8899</span>
            <span className="text-xs uppercase tracking-[0.16em] text-white/50">By appointment only</span>
          </div>
        </div>
        <p className="mt-2 text-xs text-white/50">
          Estimates shown are for illustrative purposes. Final pricing confirmed after vehicle inspection and shade selection.
        </p>
      </footer>
    </main>
  );
